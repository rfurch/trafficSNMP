...
while ((row = mysql_fetch_row(res)) != NULL) {
    ...
    strcpy(ifaceAux.peername, (row[10]) ? (row[10]) : "" ); 
    deviceAux.getrunn = (row[11]) ? atoi(row[11]) : 0;
    deviceAux.cli_acc = (row[12]) ? atoi(row[12]) : 0;
    deviceAux.vendor_id = (row[13]) ? atoi(row[13]) : 0;
    deviceAux.model_id = (row[14]) ? atoi(row[14]) : 0;
	deviceAux.snmp = (row[15]) ? atoi(row[15]) : 0;
	...
	for (i=0 ; i<shmDev->nDevices ; i++)  {
		...
		for (j=0 ; j < shmInt->nInterfaces ; j++) {
			...
			if (d->deviceId == ifs->deviceId)	
	        	printf("\n      %i:  ifid: %i  enable: %i  ifname: %s  fvarname: %s alarm_lo: %i  ", j, ifs->interfaceId, ifs->enable, ifs->name, ifs->file_var_name, ifs->alarm_lo);
			}
		}	
	fflush(stdout);	
	}
...
int dbread2 (device_list *devList)
{
char 			query[2000];
int        	 	i=0, j=0;
MYSQL_RES   	*res;
MYSQL_ROW   	row;
InterfaceData	*ifaceAux = NULL;
deviceData		*deviceAux = NULL;

if(!devList)  {
	printf("\n\n dbread: devList cannot be NULL!");
	fflush(stdout);
	return(-1);
}
...
while ((row = mysql_fetch_row(res)) != NULL)
    ...
    for (i=0, deviceFound=0 ; i < devList->nDevices && ! (deviceFound && interfaceFound) ; i++) {
		if (devList->d[i].deviceId == devId) {
        	deviceFound=1;
			deviceAux = &(devList->d[i]);

    		for (j=0, interfaceFound=0 ; j<devList->d[i].nInterfaces && !interfaceFound ; j++) {
				if (devList->d[i].ifs[j].interfaceId == ifId) {
					ifaceAux = &( devList->d[i].ifs[j]);
        			interfaceFound=1;
				}
			}
		}	
	...
	if (!deviceFound) {
	    if ( ( (devList->d) = realloc(devList->d, (( devList->nDevices ) + 1) * sizeof(deviceData) )) != NULL ) {
        memset(&(devList->d[devList->nDevices]), 0, sizeof(deviceData) );
		deviceAux = &( devList->d[devList->nDevices] );
		devList->nDevices++;
		}
      else
    ...
    if (!interfaceFound)
  	  {
      if (( (deviceAux->ifs) = realloc(deviceAux->ifs, ((deviceAux->nInterfaces) + 1) * sizeof(InterfaceData) )) != NULL )
    	{
        memset(&(deviceAux->ifs[deviceAux->nInterfaces]), 0, sizeof(InterfaceData) );
		ifaceAux = &(deviceAux->ifs[deviceAux->nInterfaces]);
		ifaceAux->alarm_status = ALARM_OK;	// to avoid false alarms on startup
		deviceAux->nInterfaces++;
		}
      else
      	perror("Error de asignacion de memoria");
	  }
	...
	deviceAux->enable = 1;
	deviceAux->deviceId = devId;
	ifaceAux->enable = 1;
    ifaceAux->interfaceId = ifId;
...
if (_verbose > 2)
    {
	for (i=0 ; i<devList->nDevices ; i++)  {
	    deviceData *d = &(devList->d[i]);
		printf("\n\n Device: (%i) %i -->  %s (%s) getrun: %i cliacc: %i, vendor:%i, model: %i ", i, d->deviceId, d->name, d->ip, d->getrunn, d->cli_acc, d->vendor_id, d->model_id);
		for (j=0 ; j < d->nInterfaces ; j++) {
			InterfaceData *ifs = &(d->ifs[j]); 	
	        printf("\n      %i:  ifid: %i  ifname: %s  fvarname: %s alarm_lo: %i  ", j, ifs->interfaceId, ifs->name, ifs->file_var_name, ifs->alarm_lo);
			}
		}	
	fflush(stdout);	
	}
